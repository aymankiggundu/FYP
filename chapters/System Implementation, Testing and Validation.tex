...
This chapter describes how Traxis was realized. Traxis' implementation, testing and validation was to make sure that it meets its set
...
Controlled simulations alone were not sufficient to capture the practical challenges faced in private shuttle operations, particularly those identified during the interview with Hills Link Express. Instead, the system design and evaluation were guided by real-world operational scenarios derived from the interview findings.
...
\textbf{User Acceptance Testing (UAT)}: Finally, the system underwent UAT with Hillsling passenger alongside the operator. Participants performed typical tasks using the system over a two-week period. Think-aloud sessions and standardized questionnaires such as the System Usability Scale (SUS) were employed to capture qualitative and quantitative feedback, validating that the system met user needs and expectations.
