...
\begin{itemize}
    \item \textbf{Clients}: A mobile application deployed on the smartphones of passengers and drivers.  
...
\begin{itemize}
    ...
    \item \textbf{MCP (Model Control Protocol)} was employed to enable AI-powered features implemented in \textbf{Python}, a high-level programming language well-suited for machine learning, data processing, and AI integration. MCP allowed the client to interact with intelligent services through well-defined APIs.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Backend \& RESTful API: Django + Django REST Framework (DRF)} \\
     ...
     \item \textbf{Real-time Features: Firebase Realtime Database / Firestore + Push Notifications} \\
...
\begin{enumerate}
    ...
    \item \textbf{External Services Layer:} Third-party services were integrated to enhance functionality:
        \begin{itemize}
            ...
            \item \textbf{MCP (Model Control Protocol)} to enable AI-driven features.
        \end{itemize}
\end{enumerate}
...
\begin{itemize}
    \item The App communicates with the backend to send ride requests, location updates, and receive notifications in real time.
    
    \item The Admin Portal queries the backend for system data, performed administrative operations, and managed configurations. All actions are logged for accountability.
    
    \item External services are accessed through secure backend APIs or directly by clients when necessary, enabling geolocation, messaging, AI processing, and transcription features.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Sprint 1 – Foundation (Weeks 1–4)} \\
          ...
          \begin{itemize}
              ...
              \item Initial setup of \textbf{MCP (Model Control Protocol)} for AI service integration, preparing APIs for future predictive ride matching and AI-based recommendations.
